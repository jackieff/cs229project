...
#seeing where the predictions are correct
correct_func = np.equal(vote_preds_f,y_test)


#plot a map to see where the correct/incorrect predictions are
...
longs = np.asarray(x_test['longitude'])
lats = np.asarray(x_test['latitude'])
...
plt.hexbin(longs[correct_func==True],lats[correct_func==True],gridsize=100,mincnt=1,edgecolors=None,cmap=plt.cm.Blues_r,zorder=20,alpha=0.6,label = 'Correct')
plt.hexbin(longs[correct_func==False],lats[correct_func==False],gridsize=100,mincnt=1,edgecolors=None,cmap=plt.cm.Reds_r,zorder=30,alpha=0.6,label = 'Incorrect')
...
plt.ylabel('Latitude')
plt.legend()
plt.title('Functionality Predictions')
plt.tight_layout()
...
a =metrics.confusion_matrix(y_test,nn_pred)
plot_confusion_matrix(a, classes_quant,title = 'Neural Network', normalize=True)
...
a =metrics.confusion_matrix(y_test,svm_pred)
plot_confusion_matrix(a, classes_quant,title = 'SVM', normalize=True)
...
a = metrics.confusion_matrix(y_test,vote_preds_f)
plot_confusion_matrix(a, classes_quant,title = 'Voting Classifier', normalize=True)

#seeing where the predictions are correct
correct_quant = np.equal(vote_preds_f,y_test)
...
plt.hexbin(longs[correct_quant==True],lats[correct_quant==True],gridsize=100,mincnt=1,edgecolors=None,cmap=plt.cm.Blues_r,zorder=20,alpha=0.6,label = 'Correct')
plt.hexbin(longs[correct_quant==False],lats[correct_quant==False],gridsize=100,mincnt=1,edgecolors=None,cmap=plt.cm.Reds_r,zorder=30,alpha=0.6,label = 'Incorrect')
...
plt.ylabel('Latitude')
plt.legend()
plt.title('Water Quantity Predictions')
plt.tight_layout()
...
plot_confusion_matrix(lr_acc, classes_qual,title = 'Logistic Regression', normalize=True)
...
a = metrics.confusion_matrix(y_test,lr_preds)
plot_confusion_matrix(a, classes_qual,title = 'Logistic Regression with L2 Penalty, C = 1'), normalize=True)
...
### RF optimizing hyperparameters
parameters = {'n_estimators':(100,75,45,20),'max_depth':(60,45,30,25)}
rf = RandomForestClassifier(criterion = 'entropy',min_samples_leaf = 5, min_samples_split = 10)
rf_cv = GridSearchCV(rf,parameters,cv=5,verbose=3)
rf_cv.fit(x_res,y_res)
...
a =metrics.confusion_matrix(y_test,rf_pred)
plot_confusion_matrix(a, classes_qual,title = 'Random Forest, 75 Estimators', normalize=True)
...
bag_best_ql = bag_cv.best_estimator_

#train and test predictions
train_bag_pred = bag_best_ql.predict(x_res)
bag_pred = bag_best_ql.predict(x_test)
...
a =metrics.confusion_matrix(y_test,bag_pred)
plot_confusion_matrix(a, classes_qual,title = 'Bagging', normalize=True)
...
for i in range(5):
    inds = k_indices[i]
    x_t,y_t = np.asarray(x_test)[inds],np.asarray(y_test)[inds]
    train_preds = bag_best_ql.predict(x_res)
    test_preds = bag_best_ql.predict(x_t)
    f1_train.append(metrics.f1_score(y_res,train_preds,average = "micro"))
    f1_test.append(metrics.f1_score(y_t,test_preds,average = "micro"))
    mcc_train.append(metrics.matthews_corrcoef(y_res,train_preds)
    mcc_test.append(metrics.matthews_corrcoef(y_t,test_preds)
...
ab_pred = ab.predict(x_test)
acc = metrics.confusion_matrix(y_test,ab_pred)
plot_confusion_matrix(acc, classes_qual,
                      title='AdaBoost', normalize=True)
...
parameters ={
'learning_rate': ["constant", "invscaling", "adaptive"],
'hidden_layer_sizes': [(136,50,2), (120,30), (60,10), (75,25,5), (136)],
'activation': ["logistic", "tanh"]
}
...
a =metrics.confusion_matrix(y_test,nn_pred)
plot_confusion_matrix(a, classes_qual,title = 'Neural Network', normalize=True)
...
svm_best_ql = svm_cv.best_estimator_

#train and test predictions
train_svm_pred = svm_best_ql.predict(x_res)
svm_pred = svm_best_ql.predict(x_test)

a =metrics.confusion_matrix(y_test,svm_pred)
plot_confusion_matrix(a, classes_qual,title = 'SVM', normalize=True)
...
for i in range(5):
    inds = k_indices[i]
    x_t,y_t = np.asarray(x_test)[inds],np.asarray(y_test)[inds]
    train_preds = svm_best_ql.predict(x_res)
    test_preds = svm_best_ql.predict(x_t)
    f1_train.append(metrics.f1_score(y_res,train_preds,average = "micro"))
    f1_test.append(metrics.f1_score(y_t,test_preds,average = "micro"))
    mcc_train.append(metrics.matthews_corrcoef(y_res,train_preds)
    mcc_test.append(metrics.matthews_corrcoef(y_t,test_preds)
...
vote_qual = VotingClassifier(estimators=[('lr', lr_best_ql), ('rf', rf_best_ql), ('nn', nn_best_ql)])
vote_qual.fit(x_res,y_res)


vote_preds_f = vote_qual.predict(x_test) #test predictions
vote_preds_t = vote_qual.predict(x_res) #train predictions

a = metrics.confusion_matrix(y_test,vote_preds_f)
plot_confusion_matrix(a, classes_qual,title = 'Voting Classifier', normalize=True)

#seeing where the predictions are correct
correct_qual = np.equal(vote_preds_f,y_test)
...
plt.hexbin(longs[correct_qual==True],lats[correct_qual==True],gridsize=100,mincnt=1,edgecolors=None,cmap=plt.cm.Blues_r,zorder=20,alpha=0.6,label = 'Correct')
plt.hexbin(longs[correct_qual==False],lats[correct_qual==False],gridsize=100,mincnt=1,edgecolors=None,cmap=plt.cm.Reds_r,zorder=30,alpha=0.6,label = 'Incorrect')
